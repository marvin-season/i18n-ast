  ...
  run: ({ ast, config }) => {
    ...
    traverse.default(ast, {
      ...
      StringLiteral(path) {
        ...
        if (includesChinese(node.value)) {
          if (t.isJSXAttribute(parent)) {
            ...
            let collection = chineseCollections.find(
              (item) => item.spec === node.value,
            );
            // 相同字符串只记录一次
            if (!collection) {
              collection = {
      ...
      Program(path) {
        ...
        node?.body?.unshift(
          babelParser.parse("import {t} from 'i18next';", {
            sourceType: "module",
          }).program.body[0],
        );
      },
